...
Overbooking é a venda de um número de produtos/serviços em maior número que o estoque atual/capacidade de serviço, sendo aplicado principalmente nas áreas de saúde e da aviação. O overbooking surgiu como uma maneira de compensar o lucro perdido derivado de clientes que não comparecem ou cancelam o serviço(no-shows) utilizando todo o estoque/capacidade de serviço disponível e para balancear a eficiência operacional e a qualidade do serviço. No contexto clínico, o principal trade-off do overbooking é entre a eficiência operacional e a qualidade do serviço: segundo Zeng et  al. (2013), o maior uso da equipe médica derivado do overbooking pode trazer benefícios como maior lucro e redução do atraso para conseguir uma consulta(appointment delay), porém, o mesmo também aumenta o tempo de espera no consultório para ser atendido(office delay) e pode até reduzir o lucro da clínica para valores críticos de panel-size.

\subsection{Métricas}

Para avaliar o modelo, serão utilizadas métricas baseadas na qualidade e eficiência de serviço, levando em consideração o overbooking e a estabilidade da clínica. A partir disso, escolhemos trabalhar com o tempo médio de espera dos pacientes até o atendimento(office delay), o tempo para obter a próxima consulta(appointment-delay) e a taxa de não comparacimento de clientes(no-show). Também serão utilizadas outras métricas como a taxa de utilização de recursos para garantir estabbiidade.

\subsection{RNN}

Uma Rede neural recorrente(RNN) é uma rede neural profunda(com diversas camadas escondidadas) treinada a partir de dados sequencias ou séries temporais com o objetivo de criar um modelo de aprendizado de máquina que faz previsões a partir de inputs sequencias. O principal difernecial das RNNs está em sua capacidade de utilizar informações de entradas prévias para influenciar o valor atual de entradas e saídas. Para uma determinada entrada, a RNN mantém um estado oculto responsável por acompanhar o contexto e agir como uma memória que armazena informações das entradas anteriores, processando a entrada atual junto com o o estado oculto da etapa anterior em cada etapa para que o RNN seja capaz de "lembrar" de dados antereiores para influenciar o valor da saída atual. No contexto clínico, utilizaremos a RNN para prever a probabilidade de no-shows e ajustar o nível de overbooking em função disso.
